...
class ResCompany(models.Model):
    ...
    def _default_report_template(self):
        ...
        report_id = report_obj.search([('model', '=', 'account.invoice'), (
            'report_name', '=', 'inflow_general_template.report_invoice_template_custom')])
        if report_id:
    ...
    def _default_report_delivery_template(self):
        report_obj = self.env['ir.actions.report']
        report_id = report_obj.search(
            [('model', '=', 'stock.picking'), ('report_name', '=', 'inflow_general_template.report_delivery_custom')])
        if report_id:
            report_id = report_id[0]
        else:
    ...
    def _default_report_sale_template(self):
        report_obj = self.env['ir.actions.report']
        report_id = report_obj.search(
            [('model', '=', 'sale.order'), ('report_name', '=', 'inflow_general_template.report_sale_order_custom')])
        if report_id:
            report_id = report_id[0]
        else:
    ...
    def _get_default_image(self, is_company, colorize=False):
        img_path = odoo.modules.get_module_resource(
            'inflow_general_template', 'static/src/img', 'avatar.png')
    ...
    def template_print1(self):
        ...
        return {
            ...
            'target': 'new',
            'url': '/report/html/%s/%s?enable_editor' % ('inflow_general_template.report_demo_template_main', self.id),
        }
